...
def get_class_fuzzy(serialized_example):
    return tf.parse_single_example(serialized_example, features={'label': tf.FixedLenFeature([], tf.string)})['label']


def main(argv):
    # Specifies datasets with scheme (name:string, count:int, soft labels:bool) for quick setup.
    datasets = {('cifar10', 1, False)}
    for current_set in datasets:
        for i in range(1, current_set[1] + 1):
            argv = []
            if current_set[1] == 1 or None:
                argv.append(os.path.join('datasets', 'SSL2', current_set[0]))
                argv.append(os.path.join('datasets', current_set[0] + '-train.tfrecord'))
            else:
                argv.append(os.path.join('datasets', 'SSL2', current_set[0] + str(i)))
                argv.append(os.path.join('datasets', current_set[0] + str(i) + '-train.tfrecord'))
            ...
            if current_set[2]:
                dataset = tf.data.TFRecordDataset(input_files).map(get_class_fuzzy, 4).batch(1 << 10)
            else:
