...
def apply(x, ops):
    if ops is None:
        return x
    # For grayscale remove color channel
    if np.shape(x)[2] == 1:
        y = Image.fromarray(np.round(127.5 * (1 + np.squeeze(x, axis=2))).clip(0, 255).astype('uint8'))
    # RGB
    else:
...
def cutout(x, level):
    ...
    for i in range(upper_coord[0], lower_coord[0]):  # for every col:
        for j in range(upper_coord[1], lower_coord[1]):  # For every row
            pixels[i, j] = c  # set the color accordingly
    return x
