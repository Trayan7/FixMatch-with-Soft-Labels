...
def find_best_checkpoint(dir, glob_term='model.ckpt-*.meta'):
    """Replacement for tf.train.latest_checkpoint.
    ...
    TAG = 'accuracy'
    folder = os.path.normpath(dir + os.sep + os.pardir)
    matches_acc = sorted(tf.gfile.Glob(os.path.join(folder, 'tf/events.out.tfevents.*')))
    assert matches_acc, 'No events files found'
    tags = set()
    accuracies = []
    for event_file in matches_acc:
        try:
