...
class Model:
    ...
    def eval_mode(self, ckpt=None):
        ...
        if ckpt is None:
            ...
            ckpt = utils.find_best_checkpoint(self.checkpoint_dir)
        else:
...
class ClassifySemi(Model):
    ...
    def eval_stats(self, batch=None, feed_extra=None, classify_op=None, verbose=True):
        ...
        for subset in ('train_labeled', 'valid', 'test'):
            ...
            # Use one hot encoding in case of hard labels
            if not FLAGS.soft_labels:
                # One-hot encoding without tensor
                labels = np.eye(self.nclass)[labels]
            accuracies.append((np.argmax(predicted, axis=1) == np.argmax(labels, axis=1)).mean() * 100)

            # Custom evaluation to extract data for automatic handling
            if subset == "test" and FLAGS.eval_ckpt != None:
                # Cross-entropy
                cross_entropy = tf2.keras.losses.CategoricalCrossentropy()
                cross = cross_entropy(labels, predicted)
                cross = cross.eval(session=tf.compat.v1.Session())
                print('cross_entropy: ' + str(cross))

                # Kulback Leibler divergence
                kl_divergence = tf2.keras.losses.KLDivergence()
                kl = kl_divergence(labels, predicted)
                kl = kl.eval(session=tf.compat.v1.Session())
                print('kl_divergence: ' + str(kl))

                # Mean center loss
                centroids_labels = np.zeros((self.nclass, self.nclass))
                centroids_features = np.zeros((self.nclass, self.nclass))
                for i in range(self.nclass):
                    weighted_features = predicted * np.ndarray.flatten(predicted[:][i])
                    centroids_features[i] = np.sum(weighted_features) / np.sum(predicted)
                    weighted_labels = labels * np.ndarray.flatten(labels[:][i])
                    centroids_labels[i] = np.sum(weighted_labels) / np.sum(labels)
                loss = np.sqrt(np.sum(np.square(centroids_features - centroids_labels)))
                print('mean_center_loss: ' + str(loss))

                # Second Best accuracy
                scnd_pred = predicted.copy()
                ...
                if len(scnd_pred_pruned) == len(scnd_label_pruned) and len(scnd_pred_pruned) > 1:
                    scnd_acc = (np.argmax(scnd_pred_pruned, axis=1) == np.argmax(scnd_label_pruned, axis=1)).mean() * 100
                else:
                    # Default value in case of failure
                    scnd_acc = 0.0
                print('\nsecond_best_acc: ' + str(scnd_acc))
                

                # Save the results in numpy arrays.
                identifier = FLAGS.identifier
                eval_directory = os.path.join('tables', 'evaluation')
                try:
                    cross_np = np.load(os.path.join(eval_directory, identifier + '_cross.npy'))
                    kl_np = np.load(os.path.join(eval_directory, identifier + '_kl.npy'))
                    center_np = np.load(os.path.join(eval_directory, identifier + '_center.npy'))
                    acc_np = np.load(os.path.join(eval_directory, identifier + '_acc.npy'))
                    snd_acc_np = np.load(os.path.join(eval_directory, identifier + '_snd_acc.npy'))
                except:
                    cross_np = list()
                    kl_np = list()
                    center_np = list()
                ...
                cross_np = list(cross_np)
                kl_np = list(kl_np)
                center_np = list(center_np)
                acc_np = list(acc_np)
                snd_acc_np = list(snd_acc_np)

                cross_np.append(cross)
                kl_np.append(kl)
                center_np.append(loss)
                acc_np.append(accuracies[-1])
                snd_acc_np.append(scnd_acc)

                cross_np = np.array(cross_np)
                kl_np = np.array(kl_np)
                center_np = np.array(center_np)
                acc_np = np.array(acc_np)
                snd_acc_np = np.array(snd_acc_np)

                cross_np = np.save(eval_directory + identifier + '_cross_np', cross_np)
                kl_np = np.save(eval_directory + identifier + '_kl_np', kl_np)
                center_np = np.save(eval_directory + identifier + '_center_np', center_np)
                acc_np = np.save(eval_directory + identifier + '_acc_np', acc_np)
                snd_acc_np = np.save(eval_directory + identifier + '_snd_acc_np', snd_acc_np)
            
        if verbose:
            self.train_print('kimg %-5d  accuracy train/valid/test  %.2f  %.2f  %.2f' %
                             tuple([self.tmp.step >> 10] + accuracies[:3]))
    ...
    def add_summaries(self, feed_extra=None, **kwargs):
        ...
        '''
        tf.summary.scalar('cross_entropy', accuracies[3])
        tf.summary.scalar('center_diff', accuracies[4])
        tf.summary.scalar('scnd_acc', accuracies[5])
        '''
