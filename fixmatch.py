...
FLAGS = flags.FLAGS
GPU_MAX_MEMORY = 7000 # in MB for each GPU
...
class AugmentPoolCTACutOut(augment.AugmentPoolCTA):
    ...
    def numpy_apply_policies(arglist):
        ...
        # PIL can not handle shape(_, _, 1).
        # First setup pass is not the same as during training.
        # Therefore 2 grayscale checks.
        if x.ndim == 3 or x.shape[3] == 1:
            # Assuming square images
            shape = (x.shape[1], x.shape[1], 1)
            return dict(image=np.stack([np.reshape(x[0], shape)] + [np.reshape([ctaugment.apply(y, cutout_policy()) for y in x[1:]], shape)]).astype('f'))
        else:
...
class FixMatch(CTAReMixMatch):
    ...
    def train(self, train_nimg, report_nimg):
        ...
        with tf.Session(config=utils.get_config()) as sess:
            # Evaluation mode often needs more free GPU memory.
            # Restrict GPU memory allocation to increase stability.
    ...
    def model(self, batch, lr, wd, wu, confidence, uratio, ema=0.999, **kwargs):
        ...
        if FLAGS.soft_labels:
            l_in = tf.placeholder(tf.float32, [batch, self.nclass], 'labels')
        else:
        ...
        if FLAGS.soft_labels:
            loss_xe = tf.losses.softmax_cross_entropy(onehot_labels=l_in, logits=logits_x)
        else:
...
if __name__ == '__main__':
    ...
    utils.setup_tf()
    flags.DEFINE_bool('soft_labels', True, 'Usage of soft labels.')
    ...
    flags.DEFINE_string('identifier', 'test', 'Used to organize checkpoint evaluations.')
    FLAGS.set_default('data_dir', './datasets')
    FLAGS.set_default('augment', 'd.d.d')
    FLAGS.set_default('dataset', 'plankton.4@1000-1')
    FLAGS.set_default('batch', 64)
    FLAGS.set_default('lr', 0.03)
    FLAGS.set_default('train_kimg', (1 << 8) * 5)
    FLAGS.set_default('data_dir', './datasets')
    #FLAGS.set_default('eval_ckpt', 'best')
    app.run(main)
