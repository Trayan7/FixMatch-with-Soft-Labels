...
class MeanTeacher(models.MultiModel):
    ...
    def model(self, batch, lr, wd, ema, warmup_pos, consistency_weight, **kwargs):
        ...
        if FLAGS.soft_labels:
            l = tf.placeholder(tf.float32, [batch, self.nclass], 'labels')
        else:
        ...
        loss = tf.losses.softmax_cross_entropy(onehot_labels=l, logits=logits_x)
...
if __name__ == '__main__':
    ...
    FLAGS.set_default('dataset', 'plankton.1@1000-1')
    FLAGS.set_default('batch', 64)
    FLAGS.set_default('lr', 0.03)
    FLAGS.set_default('train_kimg', (1 << 10))
